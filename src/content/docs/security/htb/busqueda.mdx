...
import searcher from '@assets/security/busqueda/2-searcher.png'
...
<Picture 
  src={searcher}
  priority
  alt="HTB Searcher homepage"
/>

Our target's website is a search engine, pulling from a variety of available options. Running a test search we see:

![Test search](@assets/security/busqueda/3-searchresult.png)

And if we instead select the auto redirect option:

![Auto-redirect option](@assets/security/busqueda/4-redirect.png)

The resulting search shows as:

![Auto-redirect result](@assets/security/busqueda/5-redirectresult.png)
